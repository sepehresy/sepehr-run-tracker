        ...
        if pd.notna(selected_row["Lap Details"]):
            try:
                ...
                if not lap_df.empty:
                    ...
                    # Add headers
                    ax.text(-3.6, -1, "KM", fontweight='bold')
                    ax.text(-2.6, -1, "Time", fontweight='bold')
                    ax.text(-1.6, -1, "Elev", fontweight='bold')
                    ax.text(-0.6, -1, "HR", fontweight='bold')

                    # Add data text to the left of the bars
                    for i, row in lap_df.iterrows():
                        ax.text(-3.6, i, f"{row['Distance']:.2f}", va='center', ha='left', fontweight='bold')
                        ax.text(-2.6, i, f"{row['Time']}", va='center', ha='left')
                        ax.text(-1.6, i, f"{int(row['ElevGain'])}", va='center', ha='left')
                        ax.text(-0.6, i, f"{row['HR']}", va='center', ha='left')
